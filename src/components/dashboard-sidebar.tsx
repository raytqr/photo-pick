...
export function DashboardSidebar({
    ...
    return (
        <>
            ...
            <aside className={cn(
                ...
                <div className="h-16 hidden md:flex items-center px-5 border-b border-white/5">
                    <Link href="/dashboard" className="flex items-center gap-3 group">
                        <Image src="/logo-premium.png" alt="SatSetPic" width={32} height={32} className="rounded-lg group-hover:scale-110 transition-transform duration-300" />
                            ...
                            {formatExpiry() && (
                                <span className={cn(
                                    ...
                                    formatExpiry() === "Expired" ? "bg-red-500/20 text-red-400" :
                                        formatExpiry()?.includes("days left") ? "bg-amber-500/20 text-amber-400" : "text-gray-500"
                                )}>
                                    {formatExpiry()}
                                </span>
                            )}
                ...
                <nav className="flex-1 px-3 py-2 overflow-y-auto">
                                ...
                                return (
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            pathname === link.href ? "bg-gradient-to-br from-purple-500 to-pink-500 text-white" : "bg-white/5 group-hover:bg-white/10"
                                        )}>
                                            <link.icon size={16} />
                                        </div>
                    ...
                    <div>
                        <p className="px-3 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Support</p>
                        <div className="space-y-1">
                            <Link href="https://chat.whatsapp.com/H7Ys4utow3c4Vya6TKUAih" target="_blank" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-200 font-medium group">
                                <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 group-hover:bg-white/10 transition-all duration-200"><UserPlus size={16} /></div>
                                <span className="text-sm">Join Community</span>
                            </Link>
                            {(subscriptionTier === 'Pro' || subscriptionTier === 'Unlimited') && !restricted && (
                                <Link href="https://wa.me/6285159993427" target="_blank" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 transition-all duration-200 font-medium group">
                                    <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-amber-500/10 group-hover:bg-amber-500/20"><MessageCircle size={16} /></div>
            ...
            {mobileMenuOpen && <div className="fixed inset-0 bg-black/80 z-30 md:hidden backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)} />}
        </>
    );
}
